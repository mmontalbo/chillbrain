...
updated as per: http://appengine-cookbook.appspot.com/recipe/seamless-memcache-for-datastore-get-and-query/?id=ahJhcHBlbmdpbmUtY29va2Jvb2tynwELEgtSZWNpcGVJbmRleCJAYWhKaGNIQmxibWRwYm1VdFkyOXZhMkp2YjJ0eUdnc1NDRU5oZEdWbmIzSjVJZ3hOWlcxallXTm9aU0JCVUVrTQwLEgZSZWNpcGUiQmFoSmhjSEJsYm1kcGJtVXRZMjl2YTJKdmIydHlHZ3NTQ0VOaGRHVm5iM0o1SWd4TlpXMWpZV05vWlNCQlVFa00xMAw
"""
...
class APIProxyShim(apiproxy_stub.APIProxyStub):
...
class DatastoreCachingShim(APIProxyShim):
  ...
  def _Dynamic_RunQuery(self, query, query_result):
    ...
    key = 'RunQuery:' + str(hash(query_str))
    data = memcache.get(key)
    if data is None or data['str'] != query_str:
      self.CallWrappedStub('RunQuery', query, query_result)
      try:
        memcache.set(key, {'results': query_result, 'str': query_str})
      except Exception, e:
        logging.warn("Couldn't cache query.")
        logging.warn("Reason: "+str(e));
    else:
      query_result.MergeFrom(data['results'])
